...
def gdino_sam3_box_detect(image_pil, prompt, W, H):
    """GD bbox → SAM3 box prompt → mask. Returns list of det dicts."""
...
for label, cfg in OBJECTS.items():
    ...
    for di, det in enumerate(top_dets):
        ...
        save_det_viz(
            ...
            mask=det["mask"], color=(255, 80, 0),
            extra_text="[SAM3 mask]",
        )
...
out_path = OUT_DIR / "results_sam3_fp.json"
with open(out_path, "w") as f:
    json.dump({"method": "sam3_fp", "results": results}, f, indent=2)
print(f"\nSaved → {out_path}")
