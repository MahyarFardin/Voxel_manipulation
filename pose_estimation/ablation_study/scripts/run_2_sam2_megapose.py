...
VIS_DIR    = OUT_DIR / "vis" / "sam2_megapose"
...
print("\n" + "="*60)
print("PHASE 1: Loading GD + SAM2 for detection across all objects")
print("="*60)
    ...
    for cam_name, info in camera_info.items():
        ...
        for i in order:
            ...
            save_det_viz(
                ...
                rgb_bgr, box, score, cam_name,
                mask=mask, color=(0, 180, 255),
                extra_text="[SAM2 mask shown]",
            )
            del mask
...
print("\nFreeing GD + SAM2 from GPU …")
del gd_model, sam2_model, sam2_predictor
gc.collect()
...
out_path = OUT_DIR / "results_sam2_megapose.json"
with open(out_path, "w") as f:
    json.dump({"method": "sam2_megapose", "results": results}, f, indent=2)
print(f"\nSaved → {out_path}")
